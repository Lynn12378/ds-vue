...
語義已知且有對應 1:1 技術棧的項目，依 agent 內建知識直接逐行翻新，以下為常見範例：
...
參照 [Quasar 元件轉換規範](../../instructions/quasar-components.instructions.md)，按鈕、表單與表格元件需替換為 Quasar 元件：
...
若頁面引用舊技術棧 `CM/js/ui/validation.js`，需依照 [表單驗證翻新規範](form-validation.instructions.md) 翻新為 VeeValidate + Yup + Quasar。

---

#### R2-3: Server-side 資料來源識別與補償

Server-side 資料來源翻新需於 `onMounted` 呼叫 API 取得後端資料，參照 [Server-side 資料補償規範](server-side.instructions.md)。

後端注入語法如下：
- `${xxx}`（EL 表達式，非 `${param.xxx}`）
...
以下項目視為自訂資源，依 [自訂資源翻新指引](../../references/{resourceName}.md) 翻新，無對應文件則 Fallback：
...
- 自訂資源 `{resourceName}` 無對應轉換規範文件 `.github/references/{resourceName}.md`
- 無等價技術棧

**Fallback 模式**：於 `<script setup>` 最底部統一定義 composable：
...
**Step 1: 分析與決策**
- 識別所有 R2 項目，確認翻新邊界：有規範文件則依規範翻新，無則 Fallback
- 輸出決策清單供後續步驟執行

**Step 2: 建立 Fallback Composables**
- 依 Step 1 決策清單，於 `<script setup>` 最底部建立所有 Fallback composable
- 確保後續翻新步驟可直接引用
