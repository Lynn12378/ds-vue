...
| 原始呼叫 | 替代方式 |
|---|---|
| `PageUI.setButtonsEnable(div, ['QUERY'])` | `:disable="..."` binding |
| `PageUI.createButtonArea(content, buttonConfig)` | `q-btn` 自行排版 |
| `JsUtils.getDOM(id)` | `ref` 或 Vue 原生存取 |
| `JsUtils.isArray / isObject / ...` | 原生 `Array.isArray()` 等 |
...
以上 `JsUtils` 工具函式均為直接移除項目，Vue 原生已有等價語法，無需 Fallback。

### Fallback

| 原始呼叫 | 原因 |
|---|---|
| `PageUI.loadin(configs, fixedNum)` | JSP DOM 搬移操作，改用 `<slot>` 渲染 |
| `PageUI.fixedContent(fixedNum)` | JSP frame 捲軸機制，CSS 自行處理 |
| `PageUI.resize()` | JSP 視窗高度計算，CSS `min-height: 100dvh` 取代 |
| `PageUI.createInput(type, id, setting)` | 動態建立 input DOM，改用對應 Quasar 元件 |
